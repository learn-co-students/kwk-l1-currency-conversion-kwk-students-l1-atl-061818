# Write your code here
def usd_to_eur(dollar)
  euro = (dollar.to_f*0.86)
  return euro
end 

def eur_to_usd(euro)
  dollar = (euro.to_f / 0.86)
  return dollar
end

def usd_to_jpy(dollar)
  yen = (dollar.to_f*110)
  return yen
end  

def jpy_to_usd(yen)
  dollar = (yen.to_f / 110)
  return dollar
end

def usd_to_gbp(dollar)
  pound = (dollar.to_f*0.76)
  return pound
end
  
def gbp_to_usd(pound)
  dollar = (pound.to_f / 0.76)
  return dollar
end
  
def usd_to_aud(dollar)
  australian_dollar = (dollar.to_f*1.36)
  return australian_dollar
end

def aud_to_usd(australian_dollar)
  dollar = (australian_dollar.to_f / 1.36)
  return dollar
end

puts "How much money do you want to convert?"
amount = gets.chomp().to_i

puts "Pick from the following menu:"
puts "1. USD to EUR"
puts "2. USD to JPY"
puts "3. USD to GBP"
puts "4. USD to AUD"
puts "5. EUR to USD"
puts "6. JPY to USD"
puts "7. GBP to USD"
puts "8. AUD to USD"

choice = gets.chomp().to_i


case choice
when 1
  puts usd_to_eur(amount)
when 2
  puts usd_to_jpy(amount)
when 3
  puts usd_to_gbp(amount)
when 4
  puts usd_to_aud(amount)
when 5
  puts eur_to_usd(amount)
when 6
  puts jpy_to_usd(amonut)
when 7
  puts gbp_to_usd(amount)
when 8
  puts aud_to_usd(amount)
else
  puts "Invalid input, exiting..."
end
